...
The generator lists the agents it wrote in `tools/effort-agents.json`. It removes a stale agent only when the list names it, and it refuses to overwrite an agent file it did not write, so a hand-written agent such as `poteto-agent-review.md` is never touched. Before any write or removal it refuses a symlink or directory at any path it would change, so a listed name cannot redirect a write outside `agents/`.
