...
**Maintenance.** The plugin version string now lives in three manifests: `plugins/pstack/.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, and `plugins/pstack/.codex-plugin/plugin.json`. A version bump must update all three. `.agents/plugins/marketplace.json` carries no version field. The default panel quad is enumerated only in the four panel skills (`arena`, `architect`, `how`, `interrogate`) and the `setup-pstack` sheet — keep those lines grep-identical when models change; `poteto-mode` and its references deliberately do not enumerate it. `hooks/session-start-context.md` restates skill one-liners — re-verify it whenever skill names or descriptions change. Every `commands/*.md` must carry `disable-model-invocation: true` (see 0.9.7) — `grep -L 'disable-model-invocation: true' plugins/pstack/commands/*.md` must print nothing, especially after an upstream sync adds a command. The mirror also holds (see 0.9.8): no skill with a same-named command may carry the flag — on a skill it makes the Skill tool refuse the invocation, breaking both the SessionStart mandate and the trampoline's "invoke the skill" body; only the command-less `principle-*` leaves keep it. Re-run `tests/skill-collision-repro.sh` after Claude Code upgrades; the resolution precedence it depends on is undocumented, and the script now checks both static invariants.
