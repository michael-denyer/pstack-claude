...
Dispatching those agents also needs the plugin prefix. Claude Code registers a plugin subagent as `<plugin>:<agent>`, so `subagent_type: "poteto-agent"` fails with `Agent type 'poteto-agent' not found` on Claude Code 2.1.258 and takes every fan-out delegate in `poteto-mode` and `no-comments` with it. Two substitution rules now rewrite the `poteto-agent` and `comment-sicko` dispatch sites to their `pstack:`-prefixed names, and matching denylist tokens fail a sync that stops applying them. Bare names still appear in `codex-tools.md` prose, which describes a runtime with no such subagent type.
