...
## 0.9.38 - reasoning effort per role through effort agents

Port-specific (Claude Code): upstream Cursor names effort in the model slug (`-thinking-xhigh`), which this port stripped because Claude Code sets effort separately. Claude Code's subagent frontmatter takes `effort` (`low`, `medium`, `high`, `xhigh`, `max`), and a subagent definition's effort overrides the session's, so the generator now writes one general-purpose agent (`agents/effort-<level>.md`) and one poteto-agent copy (`agents/poteto-agent-<level>.md`, poteto-agent's body verbatim) per level listed in the new `models.json` `efforts` key. The agents set no `model`; the caller still passes the role's slug. An override value may carry a level after its slug, `claude-opus-5-5 @xhigh`, per panel entry. Every role-owning skill gains a generator-stamped `## Reasoning effort` section telling the agent to dispatch such an entry through `pstack:effort-<level>` where the skill prescribes `general-purpose`, or `pstack:poteto-agent-<level>` where it prescribes `pstack:poteto-agent`; `deriveSkill` appends it on sync like the Models section. `setup-pstack` asks for an optional level per role and validates it, its Models section lists the levels, and the override-sheet preamble documents the suffix. Tests cover the agents' frontmatter and body, that the committed agents match the generator, the levels in `models.json`, and the new section's placement. The pin remains at `e8d856f`.
