...
- **Model policy** — the mapping from roles to model slugs. Defaults live in `plugins/pstack/models.json` and are stamped into each skill's Models section; the user's override sheet (`pstack-models.md`, written by `setup-pstack` at a per-runtime path) is the layer that adapts them at runtime.
