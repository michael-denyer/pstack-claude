...
- **Model policy** — the mapping from roles to model slugs. Defaults live in `plugins/pstack/models.json` and are stamped into each skill's Models section; the user's override sheet (`${CLAUDE_CONFIG_DIR:-$HOME/.claude}/pstack-models.md`, written by `setup-pstack`) is the layer that adapts them at runtime.
