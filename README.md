...
- **Models.** The `claude-*` slugs in skills are Claude defaults. On Codex, `/setup-pstack` discovers callable generic model overrides and fixed custom profiles from the current `spawn_agent` declaration. It writes model-only rows plus optional `(role, model) -> agent_type` exceptions to `~/.codex/pstack-models.md`, then copies the whole sheet into a managed `~/.codex/AGENTS.md` block. Fixed profiles own their model and effort. Start a fresh task after profile changes.
...
| Multi-model panels (arena, architect, interrogate, how-critics) | Default quad is `claude-opus-5` + `claude-fable-5` + `claude-opus-4-6` + `claude-sonnet-5`. |
