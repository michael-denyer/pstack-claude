...
Take the runners from the `architect runners` line in `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/pstack-models.md`, in place of the `arena runners` line. If the sheet or that line is missing, use the defaults in [Models](#models). Alias and rejected entries follow the runner rules in the **arena** skill's Phase A.
