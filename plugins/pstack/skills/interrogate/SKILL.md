...
Launch all four in a single message using the `Agent` tool, each with a different model. All four get the same prompt built from the template in `references/reviewer-prompt.md`.
