...
Launch all reviewers in a single message using the `Agent` tool. Use the `interrogate reviewers` line in `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/pstack-models.md`, one reviewer per entry, extending or shrinking the Reviewer A/B/C labels below to the configured entry count. If the sheet or that line is missing, use the table defaults.
