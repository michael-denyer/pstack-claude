...
**Use `subagent_type: "poteto-agent"` for any subagent you spawn inside a playbook step** (on a plugin install the type is namespaced, `pstack:poteto-agent`; the bare name errors) (code-writing delegates, ad-hoc helpers). `/poteto-mode` and `poteto-agent` route through the same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own `subagent_type` for diverse-model review; respect what the skill prescribes, don't override to `poteto-agent`.
