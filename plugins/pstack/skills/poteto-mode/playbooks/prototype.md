...
5. Verify visually on the matching surface via the driver skill (`verify` for UIs, `run` for CLIs/TUIs): screenshot each variant, drive the interaction. The eye is the test here, not an assertion.
