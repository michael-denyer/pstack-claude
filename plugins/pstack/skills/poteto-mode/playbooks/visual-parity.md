...
4. Verify each component against its baseline via image diff on the matching surface (the **verify** skill drives UIs; the **run** skill drives CLIs/TUIs). A nonzero diff is a fail; investigate the pixel delta, don't wave it through. `/loop` per component until the diff is zero.
