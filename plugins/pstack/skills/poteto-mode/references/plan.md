...
- Pass `model:` explicitly per the configured roles (the single-role default in poteto-mode's Models section covers code-writing delegations and judgment; multi-model panels run the configured quad — defaults in each panel skill's Models section, overridden via `/setup-pstack`).
