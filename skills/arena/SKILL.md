...
3. Pick the runners. Default 4 (Claude family only): `claude-opus-4-7` (extended thinking), `claude-opus-4-7` (standard), `claude-sonnet-4-6`, and `claude-haiku-4-5`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive. For cross-vendor diversity (GPT, Gemini, etc.), bridge to external CLIs from the lead via wrappers like `/gsd-review`; inside Claude Code alone, vary by tier and thinking budget.
