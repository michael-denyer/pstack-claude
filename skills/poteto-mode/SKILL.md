...
- Any prose surface → the **unslop** skill. Your reply to the user is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the **plugin-dev:skill-development** skill (Claude Code's authoring guidance for SKILL.md files).
- Before commit → the **deslop** skill (slash command `/deslop`).
- Shipping UI / IDE / CLI → drive the surface yourself. For CLIs and TUIs, the **run** skill launches and drives the app. For browser / Electron / web UIs, the **verify** skill (Claude Code's VS Code extension) launches and inspects. For bug fixes you reproduce first on the same surface yourself; hand it to the user only under the narrow exception in Bug fix step 1.
