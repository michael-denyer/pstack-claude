...
Before spawning investigators, list the available MCPs in the Claude Code environment. Use the tool list at the top of the system prompt (every MCP appears as a tool with prefix `mcp__<server>__<name>`). Otherwise read `.mcp.json` in the project / plugin, or run `claude mcp list`.
...
- Pick a subagent_type that retains MCP access. MCP-backed investigators depend on it. The source control investigator would technically be safe without MCPs, but keep modes uniform for consistency. Investigators still shouldn't write anything. That's a posture, not a sandbox.
